...
<script type="text/javascript">
    $(function(){ //页面加载完成后执行的方法
        ...
        var option="";
        $.ajax({
           url:"purchaseStore/getStore.do",//请求地址
           //data:{},//请求中携带的参数
           ...
           success:function(data){//成功后回调的函数
               ...
               //循环解析返回json  拼装option
                 for(var i=0;i<data.length;i++){  
                    option+="<option  value='"+data[i].sid+"' >"+data[i].goodsname+"</option>";
                 }
               }
           },
  ...
  <body>
  <center>
    <table id="tt" class="easyui-datagrid" title="采购管理" style="width:100%;height:600px" fitColumns='true' pagination="true" toolbar="#tb"
		 data-options="singleSelect:true,collapsible:true,url:'purchaseStore/page.do',method:'post'">
		<thead>
			<tr >
				<th data-options="field:'ordernum',align:'center',sortable:true,width:80">采购编号</th>
				...
				<th data-options="field:'innum',align:'center',width:80">商品数量</th>
				...
				<th data-options="field:'submittime',align:'center',width:80">申请时间</th>
				<th data-options="field:'state',align:'center',width:80,formatter:formatWorkingstatus">状态</th>
				<th data-options="field:'_operate',width:80,align:'center',formatter:fromatterOperation">操作</th>
			</tr>
		</thead>
	</table>
	...
	<div id="tb" style="padding:3px">
			...
			<a   class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="addStore()">批量采购</a>
...
<!--单个采购弹出框  -->		
 	<div id="indlg" class="easyui-dialog" style="width:600px;height:380px;padding:10px 20px;align:center"
		...
		<div id="indlg-buttons">
			<a   class="easyui-linkbutton" iconCls="icon-ok" onclick="storeIn()"> 采购</a>
...
<div id="dlg" class="easyui-dialog" style="width:800px;height:400px;padding:10px 20px"
	...
	//form表单数字验证
		...
		function fromatterOperation(){
			...
			+"onclick='inStore()' value='采购'><input type='button' style='background-image:linear-gradient(#f5c153,#ea920d)' onclick='destroyStore()' value='删除'>"
		}
		
			
		//单个采购保存操作
		function storeIn(){
			$('#fmin').form('submit',{//给form表单绑定submit事件
			url: url,//请求的url地址
		...
		function inStore(){
		   ...
		   var row = $('#tt').datagrid('getSelected');
			if (row.state=="已审核"){
				...
				$('#indlg').dialog('open').dialog('setTitle','采购商品');
				$('#indlg').window('center');
				//加载数据 根据datagrid里面的field的名称和form表单的名称必须一致
				$('#fmin').form('load',row);
				url = 'purchaseStore/update.do?id='+row.goodsid;
			}
			if(row.state=="待审核"){
		...
		//弹出批量采购
		function addStore(){
			$('#dlg').dialog('open').dialog('setTitle','添加采购商品');
			$('#dlg').window('center');
			$('#fm').form('clear');
			url = 'purchaseStore/input.do';
		}	
				...
				$.messager.confirm('Confirm','确认删除吗?',function(r){
					if (r){
					   ...
					   //  $.post(地址，参数，回调函数，返回值类型)
						...
						$.post('purchaseStore/del.do',{goodsid:row.goodsid},function(result){
							if (result.suc=='true'){
